<template>
  <div class="role-intro">
    <div class="intro-box">
      <h3>点击选择您要使用的知识库</h3>
      <div class="intro-con">
        <h4>"{{ agentData.agentVersionName }}"大模型知识库</h4>
        <div class="role-info">
          <img v-if="agentId" :src="agentData.agentVersionIcon" alt="" />
          <div class="content">{{ agentData.agentVersionDesc }}</div>
        </div>
        <el-button @click="useRole" type="primary" style="margin-top: 10px; width: 80px" plain>使 用</el-button>
      </div>
    </div>
  </div>
</template>
